...
class WC_upsy_Tagging
{
	...
	public function upsy_display_customer_settings() {
		...
		echo '<p>Give your upsy a name. If you are re-installing upsy, use the name from before.</p>';
	}
	...
	function load_upsy_customer_script() {
      ...
      <script>
        ...
        (function () {
          ...
          e("https://upsy-dev.shoppinghelper.net/static/upsy.js", f, document.body)
        })()
      </script>
		<?php
	}
...
// Custom action to send data to Upsy.
add_action('woocommerce_payment_complete','upsy_purchase');

// Sending data to Upsy backend.
function upsy_purchase($order_id){
	$order = new WC_Order($order_id);
	
	foreach ($_COOKIE as $key=>$val)
	{
		error_log($key.' is '.$val."<br>\n");
	}
	error_log($order);
}


add_action('woocommerce_new_order', 'upsy_purchase');
